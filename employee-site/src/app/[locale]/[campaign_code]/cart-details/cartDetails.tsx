...
const CartDetailPage: FunctionComponent<Props> = () => {
  ...
  const fetchCartItems = useCallback(
    (shareId: string) => {
      ...
      fetchShareItems(campaign_code, shareId, locale, campaignType)
        .then((cartItems) => {
  ...
  const totalPrice = useMemo(() => {
    if (campaignType === 'quick_offer_code') {
      return (
        cart?.cart?.products?.reduce((total, item) => {
          const itemPrice = item.quantity * item.product.calculated_price;
          return total + itemPrice;
        }, 0) ?? 0
      );
    }
    return giftPrice - cartBudget > 0 ? giftPrice - cartBudget : 0;
  }, [cart, campaignType]);
